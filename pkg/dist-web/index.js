const _require = require('./util'),
      ...
      assertValidOptions = _require.assertValidOptions,
...
function typeOfError(type, value) {
  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  const message = `must be of type ${toString(type)} but was ${typeOf(value)}`;
  return new TypeError(type, value, message, merge(options, {
    code: 'typeof'
  }));
}

function typeErrors(type, value) {
  ...
  if (notEmpty(_typeObject.type) && !array(_typeObject.type).find(t => typeOf(value) === t || t === 'any')) {
    return [typeOfError(_typeObject, value, {
      path
    })];
  }
  ...
  if (result === false) return [new TypeError(type, value, 'is invalid', {
    path
  })];
  return array(result).map(error => {
    if (error instanceof TypeError) {
      ...
      return new TypeError(type, value, result, {
        path
      });
    }
  });
...
function assertType(type, value) {
  ...
  if (errors) {
    throw new TypeError(type, value, `value has invalid type - there are ${errors.length} type errors`, {
      childErrors: errors
    });
  }
}

function assertTypeOptions(options) {
  let validOptionTypes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  ...
  const type = compact({
    ...
    validate: value => {
      ...
      if (options.pattern && !value.match(new RegExp(options.pattern))) {
        errors.push(new TypeError(type, value, `must match pattern ${options.pattern}`, {
          code: 'pattern'
        }));
      }
  ...
  const type = compact({
    ...
    validate: value => {
      ...
      if (options.minimum !== undefined && value < options.minimum) {
        errors.push(new TypeError(type, value, `must be at least ${options.minimum} was only ${value}`, {
          code: 'minimum'
        }));
      }

      if (options.maximum !== undefined && value > options.maximum) {
        errors.push(new TypeError(type, value, `must be no more than ${options.maximum} but was ${value}`, {
          code: 'maximum'
        }));
      }
  ...
  const type = {
    ...
    validate: value => {
      if (!values.includes(value)) {
        return new TypeError(type, value, `must be one of: ${values.join(', ')}`, {
          code: 'enum'
        });
      } else {
        return undefined;
  ...
  const type = {
    ...
    validate: value => {
      if (!(value instanceof klass)) {
        return new TypeError(type, value, `must be instanceof ${klass.name}`, {
          code: 'instanceof'
        });
      } else {
        return undefined;
...
function ObjectType(properties) {
  ...
  if (notEmpty(properties)) {
    const keyDescriptions = Object.keys(properties).map(key => {
      const meta = compact([properties[key].name, options.required.includes(key) ? 'required' : undefined]);
  ...
  const type = compact({
    ...
    validate: function validate(value) {
      ...
      if (notEmpty(options.required)) {
        ...
        if (notEmpty(missingKeys)) errors.push(new TypeError(type, value, `is missing the following required keys: ${missingKeys.join(', ')}`, {
          path,
          code: 'required'
        }));
      }
      ...
      if (options.additionalProperties === false) {
        ...
        if (notEmpty(invalidKeys)) errors.push(new TypeError(type, value, `has the following invalid keys: ${invalidKeys.join(', ')}`, {
          path,
...
function convertNested(properties) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
function NestedObject(properties) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
function ArrayType() {
  let items = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'any';
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  ...
  const type = compact({
    ...
    validate: function validate(value) {
      let path = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
      if (!Array.isArray(value)) return [typeOfError(type, value, {
        path
      })];
      const errors = [];
      ...
      if (options.minItems && value.length < options.minItems) {
        errors.push(new TypeError(type, value, `must have at least ${options.minItems} items but had only ${value.length}`, {
          path,
          code: 'minItems'
        }));
      }
      ...
      if (options.maxItems && value.length > options.maxItems) {
        errors.push(new TypeError(type, value, `must have no more than ${options.maxItems} items but had ${value.length}`, {
          path,
          code: 'maxItems'
        }));
      }
...
function Required(type) {
  ...
  const options = merge(_typeObject.options, {
    isRequired: true
  });
  return merge(_typeObject, {
    options
  });
}
