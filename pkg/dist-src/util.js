...
function assertValidOptions(options, validOptionTypes, assertConfig = {}) {
  ...
  for (const key in options) {
    ...
    if (assertConfig.additionalKeys !== true && !(key in validOptionTypes)) {
      throw new Error(`Unrecognized options key ${key}, should be one of ${Object.keys(validOptionTypes).join(', ')}`);
    }

    if (key in validOptionTypes && actualType !== expectedType) {
      throw new Error(`Type of options key ${key} is ${actualType} but needs to be ${expectedType}`);
    }
